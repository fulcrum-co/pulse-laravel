<x-layouts.dashboard title="Edit Strategy">
    <div class="max-w-2xl">
        <x-card>
            <form action="{{ route('strategies.update', $strategy) }}" method="POST" class="space-y-6">
                ...
                <div class="flex items-center gap-3">
                    <label class="flex items-center gap-2">
                        ...
                        <span class="text-sm text-gray-700">Allow upstream consultant to view this strategy</span>
                    </label>
                </div>
                @endif
        ...
        <x-card class="mt-6 border-red-200">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-900">Delete this strategy</p>
                    <p class="text-sm text-gray-500">Once deleted, this cannot be undone.</p>
                </div>
                <form action="{{ route('strategies.destroy', $strategy) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this strategy?');">
                    @csrf
                    @method('DELETE')
