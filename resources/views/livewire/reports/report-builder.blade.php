<div class="h-screen flex flex-col" x-data="{ showPublishModal: false }">
    <!-- Header -->
    ...
    >
        <div class="flex items-center justify-center min-h-screen px-4">
            <div class="fixed inset-0 bg-black/50 transition-opacity" @click="showPublishModal = false"></div>

            <div class="relative bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-xl font-semibold text-gray-900">Publish Report</h2>
                    <button @click="showPublishModal = false" class="p-1 text-gray-400 hover:text-gray-600 rounded">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                        </svg>
                    </button>
                ...
                @if($status === 'published')
                    <div class="flex items-center gap-2 p-3 bg-green-50 border border-green-200 rounded-lg mb-4">
                        ...
                        <span class="text-sm text-green-700">This report is published and accessible via the link below.</span>
                    </div>
                @else
                ...
                @if(!$reportId)
                    <div class="flex items-center gap-2 p-3 bg-yellow-50 border border-yellow-200 rounded-lg mb-4">
                        <svg class="w-5 h-5 text-yellow-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
                        </svg>
                        <span class="text-sm text-yellow-700">Save the report first before publishing.</span>
                    </div>
                @endif
                ...
                <div class="space-y-4 mb-6">
                    <div>
                        ...
                        @php
                            $publicUrl = $status === 'published' && $reportId
                                ? route('reports.public', ['token' => \App\Models\CustomReport::find($reportId)?->public_token ?? 'pending'])
                        ...
                        <div class="flex gap-2">
                            <input
                                ...
                                id="shareableLink"
                                value="{{ $publicUrl }}"
                                class="flex-1 px-3 py-2 bg-gray-50 border border-gray-300 rounded-lg text-sm {{ $status !== 'published' ? 'text-gray-400' : '' }}"
                            >
                            <button
                                @click="copyToClipboard($el.previousElementSibling.value, 'link')"
                                :disabled="'{{ $status }}' !== 'published'"
                                class="px-3 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1"
                            >
                    ...
                    <div>
                        ...
                        @php
                            $embedCode = $status === 'published' && $reportId
                                ? '<iframe src="' . route('reports.public', ['token' => \App\Models\CustomReport::find($reportId)?->public_token ?? 'pending']) . '" width="100%" height="600" frameborder="0"></iframe>'
                        ...
                        <textarea
                            ...
                            id="embedCode"
                            rows="3"
                            class="w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-lg text-sm font-mono {{ $status !== 'published' ? 'text-gray-400' : '' }}"
                        >{{ $embedCode }}</textarea>
                        <div class="flex justify-end mt-1">
                            <button
                                @click="copyToClipboard(document.getElementById('embedCode').value, 'embed')"
                                :disabled="'{{ $status }}' !== 'published'"
                                class="text-sm text-pulse-orange-600 hover:text-pulse-orange-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1"
                            >
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/>
                                </svg>
                ...
                <div class="flex gap-3">
                    ...
                    @if($status !== 'published')
                        <button
                            ...
                            class="flex-1 px-4 py-2 text-white bg-pulse-orange-500 rounded-lg hover:bg-pulse-orange-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                        >
