...
@if($viewMode === 'grid')
<a href="{{ route('resources.courses.show', $course) }}" class="block bg-white rounded-lg border border-gray-200 overflow-hidden hover:shadow-md hover:border-gray-300 transition-all h-full flex flex-col group">
    <div class="p-4 flex-1">
        ...
        <div class="flex items-center gap-2 mb-1">
            <h3 class="font-medium text-gray-900 text-sm truncate group-hover:text-pulse-orange-600 transition-colors">{{ $course->title }}</h3>
    ...
    <div class="px-4 py-2 bg-gray-50 border-t border-gray-100 flex items-center justify-between mt-auto">
        <div class="flex items-center gap-1">
            ...
            <span class="p-1 text-gray-400 rounded" title="Draft - can be edited">
...
@else
<a href="{{ route('resources.courses.show', $course) }}" class="block bg-white rounded-lg border border-gray-200 p-3 hover:shadow-sm hover:border-gray-300 transition-all flex items-center gap-4 group">
    <div class="w-10 h-10 rounded-lg bg-gradient-to-br from-pulse-orange-100 to-pulse-orange-50 flex items-center justify-center flex-shrink-0">
        <x-icon name="academic-cap" class="w-5 h-5 text-pulse-orange-600" />
    </div>
    ...
    <div class="flex items-center gap-1">
        @if($course->status === 'draft')
        <span class="p-1.5 text-gray-400 rounded" title="Draft">
