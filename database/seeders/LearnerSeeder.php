...
class LearnerSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $learnerUsers = User::where('primary_role', 'participant')->where('org_id', $organization->id)->get();
        $support_person = User::where('primary_role', 'admin')->where('org_id', $organization->id)->where('email', 'like', '%rodriguez%')->first();
        ...
        foreach ($learnerUsers as $index => $user) {
            ...
            Participant::create([
                ...
                'participant_number' => 'STU'.str_pad($index + 1, 5, '0', STR_PAD_LEFT),
                ...
                'support_person_user_id' => $support_person?->id,
            ]);
        }
    }
    ...
    {
        $allTags = [
            'good' => ['Honor Roll', 'Athletics', 'Participant Council', 'Drama Club', 'Music'],
            'low' => ['Tutoring', 'Study Group', 'Mentorship'],
            'high' => ['Priority Support', 'Weekly Check-in', 'Direct Supervisor Contact'],
        ];
