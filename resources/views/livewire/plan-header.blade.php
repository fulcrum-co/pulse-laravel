    ...
    <div class="flex items-start justify-between">
        <div class="flex-1">
                ...
                @if($editingTitle)
                    <div class="flex items-center gap-2">
                        <input type="text" wire:model="newTitle" wire:keydown.enter="saveTitle" wire:keydown.escape="cancelEditTitle"
                            ...
                            autofocus>
                        <button wire:click="saveTitle" class="text-pulse-orange-500 hover:text-pulse-orange-600 text-xs font-medium">Save</button>
                        <button wire:click="cancelEditTitle" class="text-gray-400 hover:text-gray-600 text-xs">Cancel</button>
                    </div>
                @else
            ...
            <div class="flex items-center gap-3 mb-3">
                <span class="text-xs text-gray-500">Owners and Collaborators</span>
        ...
        <div class="flex items-center gap-1">
            <a href="{{ route('plans.create') }}" class="text-xs text-gray-400 hover:text-gray-600 px-2">Create Plan +</a>
            ...
            <div class="relative" x-data="{ open: false }">
                ...
                <div x-show="open" @click.away="open = false" x-transition class="absolute right-0 mt-1 w-36 bg-white border border-gray-200 rounded-lg shadow-lg z-10 py-1">
                    <a href="{{ route('plans.edit', $plan) }}" class="block px-3 py-1.5 text-xs text-gray-700 hover:bg-gray-50">Edit</a>
                    <form action="{{ route('plans.duplicate', $plan) }}" method="POST">
                        @csrf
                        <button type="submit" class="block w-full text-left px-3 py-1.5 text-xs text-gray-700 hover:bg-gray-50">Duplicate</button>
                    </form>
                    <button @click="$dispatch('openPushPlan'); open = false" class="block w-full text-left px-3 py-1.5 text-xs text-gray-700 hover:bg-gray-50">Push to Org</button>
                    <hr class="my-1">
                    <form action="{{ route('plans.destroy', $plan) }}" method="POST" onsubmit="return confirm('Are you sure?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="block w-full text-left px-3 py-1.5 text-xs text-red-600 hover:bg-red-50">Delete</button>
                    </form>
                </div>
            </div>
    ...
    @if($showCollaboratorModal)
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-white rounded-lg shadow-xl w-full max-w-sm p-4">
                <h3 class="text-base font-semibold mb-3">Add Collaborator</h3>
                ...
                <div class="mb-3">
                    <h4 class="text-xs font-medium text-gray-500 mb-2">Current</h4>
                    <div class="space-y-1">
                ...
                <div class="flex justify-end">
                    <button wire:click="closeCollaboratorModal" class="px-3 py-1.5 text-xs text-gray-600 hover:text-gray-900">Close</button>
                </div>
            </div>
        </div>
    @endif
